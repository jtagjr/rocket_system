...
static const uint8_t rfm69_base_config[][2] =
  { { 0x02, 0x00 }, // RegDataModul: Packet mode, FSK, no shaping
    ...
    { 0x3B, (1<<5 | 6<<2 | 3)}, // RegAutoModes: Enter Rising edge FifoNotEmpty, Exit rising edge packet sent, intermediate mode TX
...
bool RFM69::powerCycle(){
  ...
  HAL_GPIO_DeInit(GPIOE, RADIO_RESET_Pin);
...
bool RFM69::readRegister(const uint8_t regAddress, uint8_t& value){
  ...
  if (HAL_OK != HAL_SPI_Receive(&hspi2, &value, 1, 10)) {
    PRINTLN("readRegister start data receive failed [0x%X]", regAddress);
    //HAL_SPI_Abort_IT(&hspi2);
    goto readExit;
  }
  result = true;
  readExit: HAL_GPIO_WritePin(SPI2_CS_RADIO_GPIO_Port, SPI2_CS_RADIO_Pin, GPIO_PIN_SET);
  return result;
}
...
bool RFM69::readRegisters(const uint8_t regAddress, uint8_t* data, const uint8_t length){
  ...
  if (HAL_OK != HAL_SPI_Receive(&hspi2, data, length, 10)) {
    PRINTLN("readRegisters start data receive failed [0x%X]", regAddress);
    //HAL_SPI_Abort_IT(&hspi2);
    goto readExit;
  }
  result = true;
...
bool RFM69::setHighPowerSettings(const bool enable){
  ...
  return writeRegister(0x5A, enable ? 0x5D : 0x55) & writeRegister(0x5C, enable ? 0x7C : 0x70);
}
