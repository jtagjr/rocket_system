...
protected:
  LSM6DSOCore(bool lowAcc);
  SetCs enable_cs;
  ClearCs disable_cs;

  HAL_StatusTypeDef spi_write(uint8_t regAddress, uint8_t* data, uint16_t len);
  HAL_StatusTypeDef spi_read(uint8_t regAddress, uint8_t* data, uint16_t len);
};
...
struct FifoStatusBits {
  uint8_t diff_fifo :2;
  uint8_t reserved :1;
  uint8_t fifo_over_latched :1;
  uint8_t counter_bdr_ia :1;
  uint8_t fifo_full_ia :1;
  uint8_t fifo_over_ia :1;
 ...
 * Permission    : RW
 *******************************************************************************/
enum LSM6DSO_STOP_ON_WTM {
  FIFO_STOP_ON_WTM_DISABLED = 0x00,
  FIFO_STOP_ON_WTM_ENABLED = 0x80,
 ...
 * Permission    : RW
 *******************************************************************************/
enum LSM6DSO_FIFO_COMPR_RT {
  FIFO_COMPR_RT_DISABLED = 0x00,
  FIFO_COMPR_RT_ENABLE = 0x40,
 ...
 * Permission    : RW
 *******************************************************************************/
enum LSM6DSO_FIFO_ODRCHG {
  FIFO_ODRCHG_DISABLED = 0x00,
  FIFO_ODRCHG_ENABLE = 0x10,
 ...
 * Permission    : RW
 *******************************************************************************/
enum LSM6DSO_FIFO_UNCOPTR_RATE {
 ...
 * Permission    : RW
 *******************************************************************************/
enum LSM6DSO_TEMPERATURE_ODR_BATCH {
...
public:
  ...
  LSM6DSO(bool lowAcc);
  bool sleep();
  bool startHighSpeedSampling(LSM6DSO_FS_XL accel_range, LSM6DSO_ODR_XL accel_odr_rate, LSM6DSO_FS_G gyro_range,
                              LSM6DSO_ODR_GYRO_G gyro_odr_rate, LSM6DSO_BDR_XL_FIFO accel_bdr_rate, LSM6DSO_BDR_GY_FIFO gyro_bdr_rate,
                              uint8_t watermark);
  bool initialize(uint8_t settings);
  ...
  bool getFifoStatus(uint16_t& numberOfSensorDataReady, FifoStatusBits& statusBits);
  uint16_t fifoRead(const uint16_t numberOfSensorDataReady, FifoData* data);
